...
impl MiniLsm {
    pub fn close(&self) -> Result<()> {
        ...
        // write back all mem-data to disk
        if !self.inner.state.read().memtable.is_empty() {
            self.inner
                .force_freeze_memtable_with_memtable(Arc::new(MemTable::create(
...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        let keep_table = |key: &[u8], table: &SsTable| -> bool {
            let mut might = true;
            if let Some(filter) = &table.bloom {
                might = filter.may_contain(farmhash::fingerprint32(key));
            }
            Self::key_within(
                key,
        ...
        for (_, sst_ids) in snapshot.levels.iter() {
            let mut level_sst_tables = Vec::with_capacity(sst_ids.len());
            for idx in sst_ids.iter() {
                let table = snapshot.sstables[idx].clone();
                if keep_table(key, &table) {
                    level_sst_tables.push(table);
                }
            }
    ...
    pub fn put(&self, _key: &[u8], _value: &[u8]) -> Result<()> {
        ...
        guard
            .memtable
            .put(_key, _value)
            .expect("memtable.put failed");
        self.check_memtabel_capacity(guard)
    }

    fn check_memtabel_capacity(&self, guard: RwLockReadGuard<Arc<LsmStorageState>>) -> Result<()> {
        if guard.memtable.approximate_size() > self.options.target_sst_size {
            drop(guard);
            let observer = self.state_lock.lock();
            let size = self.state.read().memtable.approximate_size();
            if size > self.options.target_sst_size {
                self.force_freeze_memtable(&observer)?;
                Ok(())
            } else {
                Ok(())
            }
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        let memtabble_id = self.next_sst_id();
        let new_memtable = Arc::new(MemTable::create(memtabble_id));
        self.force_freeze_memtable_with_memtable(new_memtable)?;
        Ok(())
    }

    fn force_freeze_memtable_with_memtable(&self, mem_table: Arc<MemTable>) -> Result<()> {
        // important pattern to modify Arc-wrapped data
        let mut guard = self.state.write();
        let mut snapshot = guard.as_ref().clone();
        let old_memtable = mem::replace(&mut snapshot.memtable, mem_table);
    ...
    pub fn scan(
        ...
        for (_, level_sst_ids) in snapshot.levels.iter() {
            let mut level_ssts = Vec::with_capacity(snapshot.levels.len());
            for idx in level_sst_ids.iter() {
                let table = snapshot.sstables[idx].clone();
                if range_overlap(
                    lower,
                    upper,
        ...
        Ok(FusedIterator::new(LsmIterator::new(
            TwoMergeIterator::create(
                TwoMergeIterator::create(
                    ...
                    MergeIterator::create(l0_table_iters),
                )?,

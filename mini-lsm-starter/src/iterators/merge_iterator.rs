...
impl<I: StorageIterator> MergeIterator<I> {
    pub fn create(iters: Vec<Box<I>>) -> Self {
        ...
        if iters.iter().all(|item| !item.is_valid()) {
            // all item is invalid, choose the last one as the current
            let mut iters = iters;
            return Self {
                iters: heap,
    ...
    for MergeIterator<I>
{
    ...
    /// make sure the iter always return the latest version data
    /// given the condition that the construct vec is of the sequence from new to old
    /// and the current shall be the newest
    ///
    /// while taking the next item, duplication erase is execute first,
